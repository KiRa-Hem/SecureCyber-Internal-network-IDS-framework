...
async def simulate_sensor(location):
    """Simulate a sensor sending data to the WebSocket server."""
    uri = "ws://localhost:8765/ws"
    
    try:
        async with websockets.connect(uri) as websocket:
            print(f"{location} sensor connected to WebSocket server")
        ...
        print(f"{location} sensor error: {e}")

async def main():
    """Run multiple sensors."""
    # Create tasks for each sensor location
    tasks = []
    for location in ["edge", "internal"]:
        tasks.append(asyncio.create_task(simulate_sensor(location)))
